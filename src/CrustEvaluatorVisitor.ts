...
export class CrustEvaluatorVisitor extends AbstractParseTreeVisitor<void> implements CrustVisitor<void> {
  // wc: write counter
  private wc: number = 0;
  ...
  visitExpression(ctx: ExpressionContext): void {
    if (ctx.getChildCount() == 1) {
      // If the expression is a literal, add a load constant instruction
      this.instrs[this.wc++] = { tag: "LDC", val: parseInt(ctx.getText()) };
